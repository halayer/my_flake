...
{ config, lib, pkgs, ... }:
{
  ...
  # systemd.user.services.kb-layout = {
  #   enable = true;
  #   description = "Korrektur des Tastaturtyps";
  #   script = "loadkeys de-latin1";
  #   wantedBy = [ "multi-user.target" ];
  # };

  systemd.services.vbox-resize = {
    enable = true;
    description = "VirtualBox Guest Screen Resizing";
    
    wantedBy = [ "multi-user.target" ];
    requires = [ "dev-vboxguest.service" ];
    after = [ "dev-vboxguest.service" ];
